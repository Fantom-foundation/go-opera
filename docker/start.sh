...
do
	...
	--port=5050 \
	--ipcpath /root/.opera/opera.ipc \
	--http --http.addr="0.0.0.0" --http.port=18545 --http.corsdomain="*" --http.api="eth,debug,admin,web3,personal,net,txpool,ftm,sfc" \
...
attach_and_exec() {
    ...
    do
        ...
        res=$(docker exec -i ${NAME} /opera --exec "${CMD}" attach http://127.0.0.1:18545 2> /dev/null)
        if [ $? -eq 0 ]
        then
            echo $res
            return 0
