...
func (test *snapshotTest) run() bool {
	...
	defer tx.Rollback()
	var (
		ds           = estate.NewPlainState(tx, 1)
		state        = NewWithStateReader(ds)
	...
	for sindex--; sindex >= 0; sindex-- {
		checkds := estate.NewPlainState(tx, 1)
		checkstate := NewWithStateReader(checkds)
...
func (test *snapshotTest) checkEqual(state, checkstate *StateDB) error {
	for _, addr := range test.addrs {
		...
		checkStorage := func(op string, a, b *StateDB) bool {
			if obj := a.getStateObject(addr); obj != nil {
				for key, value := range obj.dirtyStorage {
					out := b.GetState(addr, key)
					// TODO to ensure this is a required check
					if out == common.Hash(hash.Zero) {
						continue
					}

					if !checkeq(op+", GetState("+key.Hex()+")", out, value) {
						return false
					}
				}
			}
		...
		// Check storage.
		if !checkStorage("checkStorage", state, checkstate) {
			return fmt.Errorf("checkStorage err: %v", err)
		}

		if !checkStorage("checkStorage", checkstate, state) {
			return fmt.Errorf("checkStorage err: %v", err)
		}
		...
		logs, checkLogs := state.GetLogs(common.Hash{}, common.Hash{}), checkstate.GetLogs(common.Hash{}, common.Hash{})
		if !reflect.DeepEqual(logs, checkLogs) {
			return fmt.Errorf("got GetLogs(common.Hash{}, common.Hash{}) == %v, want GetLogs(common.Hash{}, common.Hash{}) == %v",
				logs, checkLogs)
		}
	}
	return nil
