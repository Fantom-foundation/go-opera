...
func (s *stateObject) GetCommittedState(key common.Hash) common.Hash {
	...
	eKeyHash := ecommon.Hash(key)
	enc, err := s.db.stateReader.ReadAccountStorage(ecommon.Address(s.Address()), uint64(1), &eKeyHash)
	if err != nil {
		s.setError(err)
		return common.Hash{}
	}
...
func (so *stateObject) Code() []byte {
	...
	code, err := so.db.stateReader.ReadAccountCode(ecommon.Address(so.Address()), uint64(1), ecommon.Hash(common.BytesToHash(so.CodeHash())))
	if err != nil {
