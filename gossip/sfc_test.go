...
//go:generate bash -c "cd ../../opera-sfc && git checkout 082a393d325a7523cc364e6db21359ab1b680904 && docker run --rm -v $(pwd)/../go-opera/gossip/contract/solc:/src/build/contracts -v $(pwd):/src -w /src node:10.5.0 bash -c 'export NPM_CONFIG_PREFIX=~; npm install --no-save; npm install --no-save truffle@5.1.4; npm run build'"
...
//go:generate bash -c "(echo -ne '\nvar ContractBinRuntime = \"'; cat contract/solc/Migrations.bin-runtime; echo '\"') >> contract/sfcproxy/contract.go"
...
func TestSFC(t *testing.T) {
	...
	_ = true &&
		...
		t.Run("Genesis SFC", func(t *testing.T) {
			...
			require.Equal(len(exp), len(hexutil.MustDecode(sfc100.ContractBinRuntime)), "genesis SFC contract (lenght)")
