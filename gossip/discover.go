...
func currentENREntry(svc *Service) *enrEntry {
	...
	return &enrEntry{
		ForkID: forkid.NewID(svc.store.GetEvmChainConfig(), common.Hash(genesisHash), uint64(svc.store.GetLatestBlockIndex()), uint64(svc.store.GetBlockState().LastBlock.Time)),
	}
}
