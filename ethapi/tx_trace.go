...
func (s *PublicTxTraceAPI) traceTx(
	...
	//_, err := evmcore.ApplyMessage(vmenv, msg, gp)
		...
		for i, tx := range block.Transactions {
			if txHash == nil || *txHash == tx.Hash() {
				...
				if tx.To() != nil && *tx.To() == sfc.ContractAddress {
					txTraces, err := s.traceTx(ctx, blockCtx, msg, state, block, tx, index, receipts[i].Status, chainConfig)
					if err != nil {
					...
					txTraces, err := s.traceTx(ctx, blockCtx, msg, state, block, tx, index, receipts[i].Status, chainConfig)
					if err != nil {
						log.Error("Cannot get transaction trace for transaction", "txHash", tx.Hash().String(), "err", err.Error())
				...
				vmConfig.Tracer = nil
				vmenv := vm.NewEVM(blockCtx, evmcore.NewEVMTxContext(msg), state, chainConfig, vmConfig)
...
func (s *PublicTxTraceAPI) Filter(ctx context.Context, args FilterArgs) (*[]txtrace.ActionTrace, error) {
	...
	blocks := make(chan rpc.BlockNumber, 2000)
