...
func TestManagmentServer(t *testing.T) {
	ctrl := gomock.NewController(t)
	defer ctrl.Finish()
	node := NewMockNode(ctrl)
	consensus := NewMockConsensus(ctrl)

	server := NewManagmentServer("localhost:55557", node, consensus, nil)
	defer server.Stop()

	client, err := NewManagmentClient("localhost:55557")
	if err != nil {
	...
	t.Run("internal_tx", func(t *testing.T) {
		assert := assert.New(t)
		ctx, cancel := context.WithCancel(context.Background())
		defer cancel()

		id := "0x70210aeeb6f7550d1a3f0e6e1bd41fc9b7c6122b5176ed7d7fe93847dac856cf"
		receiver := hash.HexToPeer(id)
		...
		_, err := client.InternalTx(ctx, &wire.InternalTxRequest{
			Amount:   2,
			Receiver: id,
		})
