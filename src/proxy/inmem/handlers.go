...
//state
type CommitHandler func(block poset.Block) (stateHash []byte, err error)

//SnapshotHandler is called by Lachesis to retrieve a snapshot corresponding to a
//particular block
type SnapshotHandler func(blockIndex int) (snapshot []byte, err error)

//RestoreHandler is called by Lachesis to restore the application to a specific
//state
type RestoreHandler func(snapshot []byte) (stateHash []byte, err error)
